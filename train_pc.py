...
# --- Local Imports ---
from create_pc_dataset import ChessDataset, OUTPUT_FILE
from dit import ChessDiT
from chess_d3pm import D3PM
from chess_utils import (
    PC_VOCAB_SIZE, PC_ABSORBING_STATE_INT, PC_SEQUENCE_LENGTH, PC_OFF_BOARD_INT,
    pc_tensor_to_fen, pc_mask_board, pc_fen_to_tensor
)
...
# Logging & Sampling
PROJECT_NAME = "chess-d3pm-pc" # Differentiate project name for PC
...
class D3PMLightning(pl.LightningModule):
    def __init__(self, learning_rate: float):
        ...
        x0_model = ChessDiT(
            vocab_size=PC_VOCAB_SIZE,
            sequence_length=PC_SEQUENCE_LENGTH,
    ...
    def sample(self, num_samples: int) -> torch.Tensor:
        initial_noise = torch.full(
            (num_samples, PC_SEQUENCE_LENGTH),
            PC_ABSORBING_STATE_INT,
            device=self.device,
            dtype=torch.long
        )
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if (global_step + 1) % self.sample_every_n_steps == 0:
            ...
            for i in range(min(self.num_samples, 2)): # Log up to 2 full samples
                ...
                viz_board_tensor = torch.where(board_tensor == PC_ABSORBING_STATE_INT, PC_OFF_BOARD_INT, board_tensor)
                img = display_pc_board_from_tensor(viz_board_tensor, show_image=False)

                if img:
                    trainer.logger.experiment.log({
                        f"generated_full_sample_{i+1}": wandb.Image(img, caption=f"Step {global_step+1}")
                    })
                reconstructed_fen = pc_tensor_to_fen(board_tensor)
                trainer.logger.experiment.log({
            ...
            initial_pc_board = pc_fen_to_tensor(self.partial_generate_fen)

            if initial_pc_board is not None:
                masked_pc_tensor = pc_mask_board(initial_pc_board, ratio=self.partial_generate_mask_ratio)
                initial_state_for_model = masked_pc_tensor.unsqueeze(0).to(pl_module.device)
                ...
                viz_masked_tensor = torch.where(masked_pc_tensor == PC_ABSORBING_STATE_INT, PC_OFF_BOARD_INT, masked_pc_tensor)
                masked_img = display_pc_board_from_tensor(viz_masked_tensor, show_image=False)
                if masked_img:
                    trainer.logger.experiment.log({
                        "partial_gen_masked_input": wandb.Image(masked_img, caption=f"Step {global_step+1} - Input Masked")
                    })
                print(f"Partial Gen Input FEN: {pc_tensor_to_fen(masked_pc_tensor)}")

                # Completed board for visualization
                viz_completed_tensor = torch.where(final_tensor_squeezed == PC_ABSORBING_STATE_INT, PC_OFF_BOARD_INT, final_tensor_squeezed)
                completed_img = display_pc_board_from_tensor(viz_completed_tensor, show_image=False)
                if completed_img:
                    trainer.logger.experiment.log({
                        "partial_gen_completed_output": wandb.Image(completed_img, caption=f"Step {global_step+1} - Output Completed")
                    })
                reconstructed_partial_fen = pc_tensor_to_fen(final_tensor_squeezed)
                trainer.logger.experiment.log({
                    "partial_gen_completed_fen": reconstructed_partial_fen
                })
                print(f"Partial Gen Output FEN: {reconstructed_partial_fen}")
            else:
                print(f"Skipping partial generation: Initial FEN '{self.partial_generate_fen}' implies promotions and cannot be processed.")
...
def main():
    if not os.path.exists(OUTPUT_FILE):
        raise FileNotFoundError(
            ...
            f"Please run create_pc_dataset.py first to generate it."
        )
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints_pc/",
        filename="chess-d3pm-pc-{epoch:02d}-{train_loss:.2f}",
        save_top_k=3,
        monitor="train_loss",
        mode="min",
    )
    ...
    print("--- Starting Training (Piece-Centric) ---")
    trainer.fit(model, dataloader)
    print("--- Training Finished (Piece-Centric) ---")
